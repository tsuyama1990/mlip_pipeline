...
class DFTConfig(BaseModel):
    ...
    pseudopotential_dir: DirectoryPath = Field(..., description="Path to pseudopotentials directory")
    ecutwfc: float = Field(40.0, gt=0, description="Wavefunction cutoff energy (Ry)")
    scf_convergence_threshold: float = Field(1e-6, gt=0, description="SCF convergence threshold (Ry)")
...
class GlobalConfig(BaseModel):
    ...
    logging_level: Literal["DEBUG", "INFO", "WARNING", "ERROR"] = Field("INFO", description="Logging level")
