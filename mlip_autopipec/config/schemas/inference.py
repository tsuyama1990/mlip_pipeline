...
class UncertaintyConfig(BaseModel):
    ...
    threshold: float = Field(5.0, gt=0, description="Gamma threshold")
    min_distance: float = Field(0.1, gt=0, description="Minimum distance for clustering")
...
class InferenceConfig(BaseModel):
    ...
    def validate_potential_exists(cls, v: FilePath) -> FilePath:
        if not v.exists():
            msg = f"Potential file {v} does not exist."
            raise ValueError(msg)
        return v
