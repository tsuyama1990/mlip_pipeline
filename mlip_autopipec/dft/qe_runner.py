...
from mlip_autopipec.exceptions import DFTCalculationError, DFTError
        ...
        try:
            with output_path.open("w") as f_out:
                subprocess.run(  # noqa: S603
                    ...
                    check=True,
                    timeout=3600 * 4,  # 4 hours hard limit
                )
        except subprocess.TimeoutExpired as e:
            msg = "DFT calculation timed out"
            raise DFTCalculationError(msg, is_timeout=True) from e
        except subprocess.CalledProcessError as e:
            stderr_output = e.stderr.decode() if e.stderr else ""
            msg = f"DFT execution failed: {stderr_output}"
            raise DFTCalculationError(msg, stderr=stderr_output) from e
