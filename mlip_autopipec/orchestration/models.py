...
class WorkflowState(BaseModel):
    ...
    """
    current_generation: int = Field(0, description="The current active learning generation (0-based).")
    status: Literal["idle", "dft", "training", "inference", "extraction"] = Field("idle", description="Current phase of the workflow.")
    pending_tasks: list[str] = Field(default_factory=list, description="List of pending task IDs or descriptions.")
...
class OrchestratorConfig(BaseModel):
    ...
    dask_scheduler_address: str | None = Field(None, description="Address of existing Dask scheduler. If None, start LocalCluster.")
