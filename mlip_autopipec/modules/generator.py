...
class PhysicsInformedGenerator:
    ...
    def _generate_for_alloy(self) -> list[Atoms]:
        ...
        logger.info(
            "Generated %d MOCK alloy structures.",
    ...
    def _create_mock_alloy_structure(self) -> Atoms:
        ...
        atoms = bulk("Cu", "fcc", a=4.0).repeat((2, 2, 2))  # type: ignore[no-untyped-call]
        symbols = ["Cu"] * 4 + ["Au"] * 4
    ...
    def _apply_strains(self, atoms: Atoms) -> list[Atoms]:
        ...
        for strain in strain_magnitudes:
            if np.isclose(strain, 1.0):
                continue
            strained_atoms = atoms.copy()  # type: ignore[no-untyped-call]
    ...
    def _apply_rattling(self, atoms: Atoms) -> list[Atoms]:
        ...
        for std_dev in std_devs:
            rattled_atoms = atoms.copy()  # type: ignore[no-untyped-call]
    ...
    def _generate_for_crystal(self) -> list[Atoms]:
        ...
        pristine_supercell = bulk(element, "fcc", a=4.0, cubic=True).repeat((3, 3, 3))  # type: ignore[no-untyped-call]
    ...
    def _create_vacancy(self, atoms: Atoms) -> Atoms:
        ...
        vacancy_atoms = atoms.copy()  # type: ignore[no-untyped-call]
        del vacancy_atoms[0]
        logger.info("Created a single vacancy structure.")
        return vacancy_atoms  # type: ignore[no-any-return]
