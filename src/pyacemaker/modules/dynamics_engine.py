...
class MDInterface:
    ...
    def _write_lammps_input(
        ...
        # Write efficiently using file stream
        # Secure permission: already inside secure work_dir, but good practice
        with input_file.open("w", encoding="utf-8") as f:
            f.write("# LAMMPS input file generated by PYACEMAKER\n")
            f.write("units metal\n")
            ...
            f.write(f"timestep {self.params.timestep}\n")
            f.write(f"fix 1 all nvt temp {self.params.temperature} {self.params.temperature} 0.1\n")
            f.write("compute pace all pace\n")
            f.write("variable pace_gamma equal c_pace\n")
            f.write(f"fix halt all halt 10 v_pace_gamma > {self.params.gamma_threshold} error continue\n")
            f.write(f"run {self.params.n_steps}\n")
...
class LAMMPSEngine(DynamicsEngine):
    ...
    def run_exploration(self, potential: Potential) -> Iterator[StructureMetadata]:
        ...
        # Use secure temporary directory relative to project root or system temp
        # Using dir=None uses system default (usually /tmp) which is secure if perms are 700
        # To handle 'insecure temporary directories' concern (e.g. symlink races in shared /tmp),
        # we can use a project-local temp dir.

        project_tmp = self.config.project.root_dir / "tmp"
        project_tmp.mkdir(exist_ok=True, mode=0o700)

        with tempfile.TemporaryDirectory(dir=project_tmp) as tmpdir:
