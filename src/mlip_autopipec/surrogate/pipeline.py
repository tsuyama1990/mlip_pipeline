...
class SurrogatePipeline:
    ...
    def _prescreen_candidates(
        ...
        if len(energies) != len(atoms_list):
            raise RuntimeError(
                f"Energy array length {len(energies)} mismatches atoms list {len(atoms_list)}"
            )
        if len(forces_list) != len(atoms_list):
            raise RuntimeError(
                f"Forces array length {len(forces_list)} mismatches atoms list {len(atoms_list)}"
            )
        ...
        for idx, forces in enumerate(forces_list):
            ...
            if forces.shape != (n_atoms, 3):
                ...
                raise RuntimeError(
                    f"Forces shape {forces.shape} invalid for {n_atoms} atoms at index {idx}"
                )
