...
class ExplorationPolicyConfig(BaseModel):
    ...
    n_local_candidates: int = Field(
        default=20, ge=1, description="Number of local candidates to generate on halt"
    )
...
class GeneratorConfig(BaseComponentConfig):
    ...
    mock_count: int = Field(
        default=2, ge=1, description="Number of structures to generate in Mock mode"
    )
...
class OracleConfig(BaseComponentConfig):
    ...
    symmetry_precision: float = Field(
        default=1e-5, gt=0.0, description="Symmetry precision for spglib/Phonopy"
    )
...
class TrainerConfig(BaseComponentConfig):
    ...
    n_active_set_per_halt: int = Field(
        default=5, ge=1, description="Number of structures to select per halt event"
    )
...
class OrchestratorConfig(BaseModel):
    ...
    max_candidates: int = Field(
        default=50, ge=1, description="Maximum number of candidates to process per cycle"
    )
