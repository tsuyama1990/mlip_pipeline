...
def test_inference_config_valid(tmp_path: Path) -> None:
    ...
    config = InferenceConfig(temperature=300.0, potential_path=potential_file)
...
def test_inference_config_invalid_potential_path(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError) as excinfo:
        InferenceConfig(temperature=300.0, potential_path=tmp_path / "non_existent.yace")
    assert "Potential file" in str(excinfo.value) or "Path does not point to a file" in str(
        excinfo.value
    )
...
def test_inference_config_negative_temp(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError):
        InferenceConfig(temperature=-10.0, potential_path=potential_file)
...
def test_inference_config_negative_pressure_npt(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError) as exc:
        InferenceConfig(
            temperature=300.0, potential_path=potential_file, ensemble="npt", pressure=-100.0
        )
...
def test_inference_config_invalid_executable(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError) as exc:
        InferenceConfig(
            temperature=300.0, potential_path=potential_file, lammps_executable=non_exec
        )
    assert "File at" in str(exc.value)
    assert "is not executable" in str(exc.value)
...
def test_inference_result_negative_gamma() -> None:
    with pytest.raises(ValidationError):
        InferenceResult(succeeded=True, max_gamma_observed=-1.0)
