...
def test_pace_driver_missing_calculator():
    ...
    with patch.object(pace_driver, 'PaceCalculator', None):
         input_data = "dummy"
         with patch("sys.stdin", StringIO(input_data)):
             # Ensure read works or we patch read
             # If read fails, it returns 1. If read succeeds but calc missing, returns 1.
             # We want to check stderr message.
             # We need valid input or robust read.
             # If input is "dummy", read fails first?
             # My run_driver reads all stdin.

             with patch("mlip_autopipec.inference.pace_driver.read_eon_geometry", return_value=MagicMock()):
...
def test_pace_driver_read_fail():
    ...
    with patch("mlip_autopipec.inference.pace_driver.read", side_effect=Exception("Bad format")):
        # Mock read_eon_geometry to fail too
        with patch("mlip_autopipec.inference.pace_driver.read_eon_geometry", side_effect=Exception("Bad EON")):
            with patch("sys.stdin", StringIO("bad data")):
