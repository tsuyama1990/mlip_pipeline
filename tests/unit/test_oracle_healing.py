...
def test_healer_reduce_mixing_beta() -> None:
    ...
    assert new_calc.parameters["mixing_beta"] == HEALER_MIXING_BETA_TARGET
    # Ensure new instance
...
def test_healer_increase_smearing() -> None:
    ...
    calc = MockEspresso(mixing_beta=HEALER_MIXING_BETA_TARGET, degauss=0.01)
    healer = Healer()
    ...
    assert new_calc.parameters["degauss"] == HEALER_DEGAUSS_TARGET
...
def test_healer_change_algo() -> None:
    ...
    calc = MockEspresso(
        mixing_beta=HEALER_MIXING_BETA_TARGET,
        degauss=HEALER_DEGAUSS_TARGET,
...
def test_healer_exhausted() -> None:
    ...
    calc = MockEspresso(
        mixing_beta=HEALER_MIXING_BETA_TARGET, degauss=HEALER_DEGAUSS_TARGET, diagonalization="cg"
    )
    healer = Healer()
