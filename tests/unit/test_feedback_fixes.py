...
def test_pseudo_dir_validation(tmp_path):
    ...
    with pytest.raises(ValidationError) as exc:
        DFTConfig(pseudopotential_dir=tmp_path / "empty", ecutwfc=30.0, kspacing=0.1, command="pw.x")
    ...
    config = DFTConfig(pseudopotential_dir=tmp_path / "valid", ecutwfc=30.0, kspacing=0.1, command="pw.x")
