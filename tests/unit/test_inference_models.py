...
def test_extracted_structure_valid() -> None:
    ...
    structure = ExtractedStructure(
        atoms=atoms, origin_uuid="1234-5678", origin_index=0, mask_radius=4.0
    )
...
def test_extracted_structure_invalid() -> None:
    ...
    with pytest.raises((ValidationError, TypeError)):
        ExtractedStructure(atoms="not_atoms", origin_uuid="1234", origin_index=0, mask_radius=4.0)
...
def test_extracted_structure_invalid_atoms_type() -> None:
    ...
    with pytest.raises((ValidationError, TypeError)) as exc:
        ExtractedStructure(atoms=123, origin_uuid="uuid", origin_index=0, mask_radius=1.0)
...
def test_extracted_structure_edge_cases() -> None:
    # Empty UUID
    s = ExtractedStructure(atoms=Atoms(), origin_uuid="", origin_index=0, mask_radius=1.0)
