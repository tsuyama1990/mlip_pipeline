...
def test_dft_config_validation(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError):
        DFTConfig(command="pw.x", pseudopotential_dir=pseudo_dir, scf_convergence_threshold=0)

    # Test invalid mixing beta
    with pytest.raises(ValidationError):
        DFTConfig(command="pw.x", pseudopotential_dir=pseudo_dir, mixing_beta=1.5)

    # Test missing pseudo dir
    with pytest.raises(ValidationError):
        DFTConfig(command="pw.x", pseudopotential_dir=tmp_path / "non_existent")

    # Test negative ecutwfc
    with pytest.raises(ValidationError):
        DFTConfig(command="pw.x", pseudopotential_dir=pseudo_dir, ecutwfc=-10)


def test_global_config_validation() -> None:
    # Valid config
    config = GlobalConfig(
        project_name="TestProject", database_path=Path("test.db"), logging_level="DEBUG"
    )
    ...
    with pytest.raises(ValidationError):
        GlobalConfig(
            project_name="TestProject", database_path=Path("test.db"), logging_level="INVALID"
        )

    # Empty project name
    with pytest.raises(ValidationError):
        GlobalConfig(project_name="", database_path=Path("test.db"))
