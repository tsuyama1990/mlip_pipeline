...
def test_write_inputs(lammps_config, silicon_structure, md_params, tmp_path):
    runner = LammpsRunner(lammps_config, base_work_dir=tmp_path)
    work_dir = tmp_path / "job_01"
    work_dir.mkdir()

    runner._write_inputs(work_dir, silicon_structure, md_params)
...
@patch("ase.io.read")
def test_run_success(mock_read, mock_run, lammps_config, silicon_structure, md_params, tmp_path):
    ...
    result = runner.run(silicon_structure, md_params)
...
@patch("mlip_autopipec.infrastructure.io.run_subprocess")
def test_run_failed(mock_run, lammps_config, silicon_structure, md_params, tmp_path):
