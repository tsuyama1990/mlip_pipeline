...
├── **orchestration/**
│   ├── **__init__.py**
│   ├── **loop.py**            # Orchestrator class
│   ├── **state.py**           # WorkflowState class
│   └── **candidate.py**       # Candidate selection logic
...
**`Orchestrator` (in `orchestration/loop.py`)**
-   **Responsibilities**:
    -   Initialize the loop from `WorkflowConfig`.
    -   Maintain `WorkflowState` (current iteration, current potential path).
    -   Execute the state machine:
        1.  `run_exploration()`: Calls `LammpsRunner`.
        2.  `process_candidates()`: If halted, calls `CandidateProcessor`.
        3.  `run_oracle()`: Calls `QERunner`.
        4.  `run_training()`: Calls `PacemakerWrapper`.
...
**`WorkflowState` (in `orchestration/state.py`)**
-   **Fields**:
    -   `iteration`: int
    -   `latest_potential_path`: Path
    -   `dataset_path`: Path
    -   `status`: Enum (IDLE, EXPLORING, CALCULATING, TRAINING)

## 4. Implementation Approach

1.  **State Management**:
    -   Implement `WorkflowState` using Pydantic. Ensure it can be serialized to JSON.
    -   Implement logic to load state on startup (Resume capability).

2.  **Selection Logic**:
    -   Wrap `pace_activeset` command. It takes a pool of structures and an existing dataset/potential, and returns indices of structures that maximize the information gain.

3.  **The Loop**:
    -   Implement the `while` loop in `Orchestrator.run()`.
    -   Use `shutil` to organize directories (`active_learning/iter_001/`, etc.).

4.  **Integration**:
    -   Wire up `LammpsRunner` -> `CandidateProcessor` -> `QERunner` -> `DatabaseManager` -> `PacemakerWrapper`.
