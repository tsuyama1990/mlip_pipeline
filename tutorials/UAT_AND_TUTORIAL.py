...
def imports_and_setup(os, sys, Path):
    ...
    try:
        ...
        print(f"Details: {e}")
        # Define dummy classes to prevent NameErrors in type hints/returns if needed
        # but since we initialized to None above, we are safe from UnboundLocalError.
        # We define them as dummy classes only if types are strictly needed by Marimo signature inspection,
        # but None is usually safer for logic checks.
    ...
    if not HAS_PYACEMAKER:
        mo.md(
            ...
            **Installation Instructions:**
            ...
            2.  Run the sync command in the project root:
    ...
    if HAS_PYACEMAKER:
        ...
        if IS_CI:
            for element, filename in pseudos.items():
                ...
                if not path.exists():
                    print(f"WARNING: Creating dummy pseudopotential for {element}: {filename}. DO NOT USE FOR REAL PHYSICS.")
...
def phase_1_markdown(mo):
    mo.md(
        ...
        *   **Extrapolation Grade ($\gamma$):**
            This is the "Uncertainty Score" of the potential for a given atomic configuration. It is derived from the D-Optimality criterion of the linear basis set used in ACE.
            *   **Scientific Meaning:** It represents how far the current configuration is from the training set in the descriptor space.
            *   **Role in Active Learning:** If $\gamma > \gamma_{threshold}$, the potential is extrapolating (guessing) rather than interpolating. We halt the simulation immediately to avoid unphysical behavior and send this "novel" structure to the Oracle (DFT) for labeling.

        *   **Active Set Optimization:**
            Instead of training on every single snapshot from an MD trajectory (which would be redundant and slow), we select an **Optimal Active Set**. We use the MaxVol algorithm or D-Optimality to select a subset of structures that maximizes the information content (determinant of the information matrix). This ensures data efficiency.
...
def phase_3_markdown(mo):
    mo.md(
        ...
        *   It allows the system to "hop" between stable states, extending the timescale to real-world relevance.
        ...
        The plot below shows the simulated rise in the Order Parameter (0 = Disordered, 1 = Ordered L10) over time.
        """
    )
    return
...
def conclusion_markdown(mo):
    mo.md(
        ...
        **Next Steps:**
        *   Try running this notebook in **Real Mode** by setting `CI=False` and ensuring Quantum Espresso (`pw.x`) and LAMMPS (`lmp`) are in your PATH.
        *   Adjust the `config` to simulate a different material system.
