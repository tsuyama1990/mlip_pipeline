...
def setup_configuration(HAS_PYACEMAKER, IS_CI, PYACEMAKERConfig, Path, mo, tempfile):
    ...
    if HAS_PYACEMAKER:
        ...
        # In Mock Mode, we rely on PYACEMAKER_SKIP_FILE_CHECKS to bypass existence checks.
        # In Real Mode, we must verify they exist.
        if not IS_CI:
            missing = []
...
def phase_3_markdown(mo):
    mo.md(
        ...
        *   It allows the system to "hop" between stable states, extending the timescale to real-world relevance.
        ...
        The plot below shows the simulated rise in the **Order Parameter**.
        *   **0.0**: Disordered (Random alloy)
        *   **1.0**: Perfectly Ordered (L10 layered structure)
        """
    )
    return
